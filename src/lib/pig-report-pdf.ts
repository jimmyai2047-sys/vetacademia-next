// Pig breeder-unit bank-format PDF generator (pdf-lib, server-side only).
// Mirrors the approved NLM-EDP Pig PDF: cover, auto index, introduction,
...
export interface PigReportInput {
  cover: CoverDetails;
...
const LBL: Record<string, { en: string; hi: string }> = {
  ...
  dietary: { en: "8. Dietary Management of Pig", hi: "8. सूअरों का आहार प्रबंधन" },
  water: { en: "9. Water", hi: "9. पानी" },
  diseases: { en: "10. Diseases of pig and their prevention", hi: "10. सूअरों के रोग एवं रोकथाम" },
...
export async function buildPigReport(input: PigReportInput): Promise<Uint8Array> {
  ...
  const breedName = input.breedName == null ? "Large White Yorkshire" : input.breedName;
  const ctx = new Ctx();
  await ctx.init(lang);
  ...
  const unitLabel0 = "(" + rates.sows + " + " + pigCosts(rates).flock[0].boars + ") Pig breeder unit";
  ctx.headerTitle = input.reportTitle == null ? "Pig Breeder Unit Project Report " + unitLabel0 : input.reportTitle;
  ...
  const costs = pigCosts(rates);
  const fin = pigFinance(rates);
  ...
  ctx.y = ctx.curH - 150;
  const unitLabel = "(" + rates.sows + " + " + f1.boars + ") Pig breeder unit";
  ...
  try {
    const sketchPath = require("path").join(process.cwd(), "public", "sketches", "pig.png");
    if (require("fs").existsSync(sketchPath)) {
      ...
      drawPigSketch(ctx, ctx.curW / 2, ctx.y - 40, 88 * 2);
      ctx.y -= 180;
    }
  } catch {
    drawPigSketch(ctx, ctx.curW / 2, ctx.y - 40, 88 * 2);
  ...
  const breed = findBreed("Pig", breedName);
  const purpose = purposesOf("Pig").filter(function (p) {
    return p.purpose.indexOf("Dual") >= 0;
  })[0] || purposesOf("Pig")[0];

  ctx.subTitle(ctx.t("projectDescription"));
  ctx.para("Pig is a multi-functional animal and plays a significant role in the economy and nutrition of landless, small and marginal farmers in the country. Pig rearing is an enterprise practised by a large section of the rural population. Pig are among the main meat-producing animals in India and have a huge domestic demand.");
  ctx.para("Benefits of commercial pig farming:");
  const benList = purpose ? purpose.benefits.slice(0, 12) : [];
  for (let bi = 0; bi < benList.length; bi++) ctx.bullet(benList[bi], ">");
  ctx.para("At present, pig rearing under intensive and semi-intensive systems for commercial production is gaining momentum. A number of commercial pig farms have been established in different regions of the country and state.");
  ...
  ctx.para("The Pig Farm will be constructed in the village of " + loc.farmVillage + ", Tehsil: " + loc.tehsil + ", District: " + loc.district + ". The given location is " + loc.highwayDistKm + " from the " + loc.highway + " and " + townStr + ", where an assured year-round market is available. It is easily accessible from the main road due to the availability of a good paved road.");

  {
    const breedParaPig = breed
      ? "For this project site, the " + breed.breed + " pig breed will be reared and improved for germplasm development. " + breed.note + " Origin: " + breed.origin + ". Male body weight " + breed.maleWtKg + " kg and female " + breed.femaleWtKg + " kg."
      : "For this project site, the " + breedName + " pig breed will be reared and improved for germplasm development.";
    ctx.subTitleWithPara("breed", breedParaPig);
  }
  ...
  const byCatPig = diseasesByCategory("Pig");
  ctx.subTitle(ctx.t("diseases"));
  ctx.para("Normally pig resist many diseases, but intensive rearing of large numbers at one place can spread disease, lowering production and raising mortality. Piglets will be dewormed from 1 month of age monthly up to 6 months; ecto-parasites treated carefully; proper vaccination against viral and bacterial diseases practised.");
  for (const cat of Object.keys(byCatPig)) {
    ctx.categoryLabel(cat + ":");
    const catRows = byCatPig[cat].map(function (d) { return [d.disease, d.symptoms, d.prevention]; });
  ...
  {
    const vetText = loc.vetHospital + " of the Department of Animal Husbandry is available near the proposed pig farm. Technical guidance: " + loc.vetOfficer + "; " + loc.pvk + ".";
  ...
  const boarsN = f1.boars;
  const totA = costs.totalAnimals;
  const pigletN = f1.alivePigletsM + f1.alivePigletsF;
  ...
  // Keep A. Assumptions and I. Techno together on same page
  {
    ...
    ['7', 'Farrowing interval', 'Months', String(rates.farrowingIntervalMonths)],
    ['8', 'Number of farrowing', 'Per Year', String(Math.round((12 / rates.farrowingIntervalMonths) * 10) / 10)],
    ['9', 'Farrowing Percentage', '%', String(rates.farrowingPct)],
    ['10', 'Average litter size', 'Number', String(rates.litterSize)],
    ['11', 'Sex Ratio', 'Ratio', '01:01'],
    ['12', 'Mortality Rate of piglets', '%', String(rates.pigletMortalityPct)],
    ['13', 'Alive piglets in a Year', 'Number', String(pigletN)],
    ['14', 'Saleable age of piglets', 'Months', '12'],
  ...
  ctx.table(['S.No', 'Particulars', 'Unit', 'Quantity'], [
    ['1', 'Space per boar', 'Sq.ft', '20'],
    ['2', 'Space per sow', 'Sq.ft', '10'],
    ['3', 'Space for piglets', 'Sq.ft', '4'],
    ['4', 'Space for sick animals', 'Sq.ft', '20'],
    ['5', 'Construction of shed', 'Rs./Sq. ft.', String(rates.constructionRate)],
    ['6', 'Cost of each sow with transport', 'Rs./Sow', String(rates.sowCost)],
    ['7', 'Cost of each boar with transport', 'Rs./Boar', String(rates.boarCost)],
    ...
    ['13', 'Concentrate piglets per month', 'Kg (@150g/day)', '4.5'],
  ...
  ctx.table(['S.No', 'Particulars', 'Unit', 'Quantity', 'Rs./Unit'], [
    ['1', 'Sale price of Boars (12 month)', 'Rs./Boar', '1', String(rates.malePigletPrice)],
    ['2', 'Sale price of Sows (12 month)', 'Rs./Sow', '1', String(rates.femalePigletPrice)],
  ...
  const capUnits = ['Rs./Sow', 'Rs./Boar', 'Sq.ft', 'Sq.ft', 'Sq.ft', 'Sq.ft', 'Rs./Equipment', 'Rs./machine', '% of animal cost', 'Rs./Animal'];
  ...
  const ycols = function (fn: (f: PigFlockYear) => string) { return fy.map(function (f) { return fn(f); }); };
  const flockHeaders = ['S.No', 'Particular'].concat(yrCols);
  const flockWidths = [35, 200, 45, 45, 45, 45, 45, 45];
  const flockRows = [
    ['1', 'Numbers of farrowing per year'].concat(ycols(function (f) { return String(f.farrowings); })),
    ['2', 'Piglets born (Male)'].concat(ycols(function (f) { return String(f.pigletsBornM); })),
    ['3', 'Piglets born (Female)'].concat(ycols(function (f) { return String(f.pigletsBornF); })),
    ['4', 'Mortality of male piglets'].concat(ycols(function (f) { return String(Math.round(f.pigletsBornM * rates.pigletMortalityPct / 100)); })),
    ['5', 'Mortality of female piglets'].concat(ycols(function (f) { return String(Math.round(f.pigletsBornF * rates.pigletMortalityPct / 100)); })),
    ['6', 'Male piglets for sale'].concat(ycols(function (f) { return String(f.saleM); })),
    ['7', 'Female piglets for sale'].concat(ycols(function (f) { return String(f.saleF); })),
    ['8', 'Total piglets for sale'].concat(ycols(function (f) { return String(f.saleM + f.saleF); })),
  ];
  ...
  ctx.para('(Piglets born in I year will be sold in II Year and so on)');
  // keep profitability heading with income table
  if (ctx.y < 320) ctx.newPage();
  ctx.subTitle(ctx.t('profitability'));
  const maleAmt = f1.saleM * rates.malePigletPrice;
  const femaleAmt = f1.saleF * rates.femalePigletPrice;
  ...
  const incRows = [
    ['1', 'Sale of male pig', 'Boarling', fmt(rates.malePigletPrice), String(f1.saleM)].concat(incY(maleAmt, true)),
    ['2', 'Sale of female pig', 'Sowling', fmt(rates.femalePigletPrice), String(f1.saleF)].concat(incY(femaleAmt, true)),
    ['3', 'Sale of pig manure', 'Tonnes', fmt(rates.manureRatePerTonne), String(Math.round(costs.manureTonnes * 1000) / 1000)].concat(incY(manureAmt, false)),
  ...
  const expRows = [
    ...
    ['3', w[2].label, 'Kg (@150g/day)', fmt(rates.concentrateRate), fmt(costs.pigletConcentrateKg)].concat(expY(w[2].amount)),
  ...
  ctx.para('TC = FC + VC1 x Q + VC2 x Q x Q, where Q is saleable piglets per year. P = Rs. ' + fmt(Math.round(P * 100) / 100) + ' per piglet, FC = Rs. ' + fmt(Math.round(FC)) + ' (capital / ' + years + ' years), VC1 = Rs. ' + fmt(Math.round(VC1 * 100) / 100) + ' per piglet, VC2 = 0.0002.');
  ctx.para('Lower break-even Q1 = ' + beQ1 + ' piglets (Rs. ' + fmt(be.sales1 == null ? 0 : be.sales1) + '). Upper break-even Q2 = ' + fmt(Math.round(beQ2)) + ' piglets. Farm capacity is ' + saleQty + ' piglets per year.');
...
function drawBreakEvenChart(ctx: Ctx, P: number, FC: number, VC1: number, VC2: number, Q1: number, capacity: number): void {
  ...
  page.drawText('Piglets per year (Q)', { x: x0 + plotW / 2 - 40, y: y0 - 26, size: 8, font: f, color: rgb(0, 0, 0) });
...
function drawPigSketch(ctx: Ctx, cx: number, cy: number, size: number): void {
  const page = ctx.cur;
  const s = size / 100;
  const g = rgb(0.14, 0.14, 0.14);
  // Simple pig head silhouette using lines/curves approximation
  // Head oval
...
export function samplePigInput(): PigReportInput {
  return {
    language: 'en',
