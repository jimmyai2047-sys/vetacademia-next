// Goat breeder-unit bank-format PDF generator (pdf-lib, server-side only).
...
import { GOAT_DEFAULTS, GoatProjectInput, GoatFlockYear, goatCosts, goatFinance } from "./goat-engine";
...
export interface GoatReportInput {
  cover: CoverDetails;
...
const LBL: Record<string, { en: string; hi: string }> = {
  ...
  rearingSystem: { en: "4. Preferred goat rearing system: Semi-Intensive System", hi: "4. बकरी पालन प्रणाली: अर्ध-सघन प्रणाली" },
...
export async function buildGoatReport(input: GoatReportInput): Promise<Uint8Array> {
  const lang = input.language == null ? "en" : input.language;
  const rates = Object.assign({}, GOAT_DEFAULTS, input.rates == null ? {} : input.rates);
  const breedName = input.breedName == null ? "Sirohi" : input.breedName;
  const ctx = new Ctx();
  await ctx.init(lang);
  ...
  const costs = goatCosts(rates);
  const fin = goatFinance(rates);
  ...
  try {
    ...
    if (require("fs").existsSync(sketchPath)) {
      ...
      drawGoatSketch(ctx, ctx.curW / 2, ctx.y - 40, 88 * 2);
      ctx.y -= 180;
    }
  } catch {
    drawGoatSketch(ctx, ctx.curW / 2, ctx.y - 40, 88 * 2);
  ...
  const breed = findBreed("Goat", breedName);
  ...
  const ycols = function (fn: (f: GoatFlockYear) => string) { return fy.map(function (f) { return fn(f); }); };
  ...
  ctx.para('Note: Year-1 DSCR is low — first-year kid crop sold in Year-2 (moratorium for first year).', 9);
  ctx.land = false;
...
function drawGoatSketch(ctx: Ctx, cx: number, cy: number, size: number): void {
  const page = ctx.cur;
  const s = size / 100;
...
export function sampleGoatInput(): GoatReportInput {
  return {
    language: 'en',
    breedName: 'Sirohi',
...
export async function buildSampleGoatPdf(outPath: string): Promise<{ pages: number; bytes: number }> {
  const bytes = await buildGoatReport(sampleGoatInput());
