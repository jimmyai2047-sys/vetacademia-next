// Sheep breeder-unit bank-format PDF generator (pdf-lib, server-side only).
// Mirrors the approved NLM-EDP Sheep PDF: cover, auto index, introduction,
...
import { SHEEP_DEFAULTS, SheepProjectInput, SheepFlockYear, sheepCosts, sheepFinance } from "./sheep-engine";
...
export interface SheepReportInput {
  cover: CoverDetails;
...
const LBL: Record<string, { en: string; hi: string }> = {
  ...
  dietary: { en: "8. Dietary Management of Sheep", hi: "8. बकरियों का आहार प्रबंधन" },
  water: { en: "9. Water", hi: "9. पानी" },
  diseases: { en: "10. Diseases of sheep and their prevention", hi: "10. बकरियों के रोग एवं रोकथाम" },
...
export async function buildSheepReport(input: SheepReportInput): Promise<Uint8Array> {
  const lang = input.language == null ? "en" : input.language;
  const rates = Object.assign({}, SHEEP_DEFAULTS, input.rates == null ? {} : input.rates);
  const breedName = input.breedName == null ? "Chokla" : input.breedName;
  const ctx = new Ctx();
  await ctx.init(lang);
  ...
  const unitLabel0 = "(" + rates.ewes + " + " + sheepCosts(rates).flock[0].rams + ") Sheep breeder unit";
  ctx.headerTitle = input.reportTitle == null ? "Sheep Breeder Unit Project Report " + unitLabel0 : input.reportTitle;
  ...
  const costs = sheepCosts(rates);
  const fin = sheepFinance(rates);
  ...
  ctx.y = ctx.curH - 150;
  const unitLabel = "(" + rates.ewes + " + " + f1.rams + ") Sheep breeder unit";
  ...
  try {
    const sketchPath = require("path").join(process.cwd(), "public", "sketches", "sheep.png");
  ...
  const breed = findBreed("Sheep", breedName);
  const purpose = purposesOf("Sheep").filter(function (p) {
    return p.purpose.indexOf("Dual") >= 0;
  })[0] || purposesOf("Sheep")[0];

  ctx.subTitle(ctx.t("projectDescription"));
  ctx.para("Sheep is a multi-functional animal and plays a significant role in the economy and nutrition of landless, small and marginal farmers in the country. Sheep rearing is an enterprise practised by a large section of the rural population. Sheep are among the main meat-producing animals in India and have a huge domestic demand.");
  ctx.para("Benefits of commercial sheep farming:");
  const benList = purpose ? purpose.benefits.slice(0, 12) : [];
  for (let bi = 0; bi < benList.length; bi++) ctx.bullet(benList[bi], ">");
  ctx.para("At present, sheep rearing under intensive and semi-intensive systems for commercial production is gaining momentum. A number of commercial sheep farms have been established in different regions of the country and state.");
  ...
  ctx.para("The Sheep Farm will be constructed in the village of " + loc.farmVillage + ", Tehsil: " + loc.tehsil + ", District: " + loc.district + ". The given location is " + loc.highwayDistKm + " from the " + loc.highway + " and " + townStr + ", where an assured year-round market is available. It is easily accessible from the main road due to the availability of a good paved road.");

  {
    const breedParaSheep = breed
      ? "For this project site, the " + breed.breed + " sheep breed will be reared and improved for germplasm development. " + breed.note + " Origin: " + breed.origin + ". Male body weight " + breed.maleWtKg + " kg and female " + breed.femaleWtKg + " kg."
      : "For this project site, the " + breedName + " sheep breed will be reared and improved for germplasm development.";
  ...
  ctx.para("Providing simple sheds made with low-cost housing materials for sheep is sufficient to achieve their optimal production capacity. The houses will be semi-closed type with North-South orientation and a long axis in East-West direction. There will be separate houses for dry, pregnant, lactating, sick, rams and lambs. The shed will be built on an elevated area on a raised platform (about 1 meter high) to prevent water stagnation. Clean drinking water, asbestos roof, concrete floor with drainage, proper ventilation with concrete and wire-mesh side walls, and separate feeders and water troughs will be provided.");
  ...
  const byCatSheep = diseasesByCategory("Sheep");
  ctx.subTitle(ctx.t("diseases"));
  ctx.para("Normally sheep resist many diseases, but intensive rearing of large numbers at one place can spread disease, lowering production and raising mortality. Lambs will be dewormed from 1 month of age monthly up to 6 months; ecto-parasites treated carefully; proper vaccination against viral and bacterial diseases practised.");
  for (const cat of Object.keys(byCatSheep)) {
    ctx.categoryLabel(cat + ":");
    const catRows = byCatSheep[cat].map(function (d) { return [d.disease, d.symptoms, d.prevention]; });
  ...
  {
    const vetText = loc.vetHospital + " of the Department of Animal Husbandry is available near the proposed sheep farm. Technical guidance: " + loc.vetOfficer + "; " + loc.pvk + ".";
  ...
  ctx.subTitleWithPara("market", "People in and around the project area prefer sheep meat (chevon) to any other meat or chicken; it is on the menu at all ceremonies, so market demand is high. Purchasing power is rising and non-vegetarian food is now almost essential in the diet. Chevon availability is less than demand; investment is smaller and risk lower than dairy. The state leads in sheep population and the demand for sheep meat is rising faster than the sheep population.");

  ctx.subTitleWithPara("export", "The scope for exports too is huge; however, for selling sheep meat abroad, one has to adhere to strict phytosanitary conditions and standards of the respective nation.");
  ...
  ctx.bullet('High and ready market for sheep meat.');
  ctx.bullet('Confirmed ever-increasing market price.');
  ctx.para('Weakness:');
  ctx.bullet('Organised large-scale sheep farming is not yet fully established.');
  ...
  ctx.table(['S. No.', 'Parameter', 'Details'], [
    ['1', 'Animal Type', 'Sheep (Small ruminant)'],
    ['2', 'Breed', breedName],
    ...
    ['8', 'Lambing Interval', String(rates.lambingIntervalMonths) + ' months'],
    ...
    ['13', 'Floor Space (Covered)', 'Ewes (' + ewesN + '): 10 Sq. ft = ' + fmt(costs.coveredEwe) + '; Lambs (' + lambN + '): 4 Sq. ft = ' + fmt(costs.coveredLambs) + '; Ram (' + ramsN + '): 20 Sq. ft = ' + fmt(costs.coveredRam) + '; Sick (' + sickN + '): 20 Sq. ft = ' + fmt(costs.coveredSick) + '; Total ' + fmt(costs.coveredTotal) + ' Sq. ft'],
  ...
  // Keep A. Assumptions and I. Techno together on same page
  {
    ...
    ['1', 'Breed of Sheep', '', 'Recognised Indian Breed (' + breedName + ')'],
    ['2', 'System of rearing', '', 'Semi-intensive'],
    ['3', 'Number of ewes', 'Number', String(ewesN)],
    ['4', 'Number of Rams', 'Number', String(ramsN)],
    ['5', 'Total sheep (Ram+Ewe)', 'Number', String(totA)],
    ['6', 'Age at maturity', 'Months', '11'],
    ['7', 'Lambing interval', 'Months', String(rates.lambingIntervalMonths)],
    ['8', 'Number of lambing', 'Per Year', String(Math.round((12 / rates.lambingIntervalMonths) * 10) / 10)],
    ['9', 'Lambing Percentage', '%', String(rates.lambingPct)],
    ['10', 'Average litter size', 'Number', String(rates.litterSize)],
    ['11', 'Sex Ratio', 'Ratio', '01:01'],
    ['12', 'Mortality Rate of lambs', '%', String(rates.lambMortalityPct)],
    ['13', 'Alive lambs in a Year', 'Number', String(lambN)],
    ['14', 'Saleable age of lambs', 'Months', '12'],
  ...
  ctx.para('Note: Year-1 DSCR is low — first-year lamb crop sold in Year-2 (moratorium for first year).', 9);
  ctx.land = false;
  ...
  ctx.para('TC = FC + VC1 x Q + VC2 x Q x Q, where Q is saleable lambs per year. P = Rs. ' + fmt(Math.round(P * 100) / 100) + ' per lamb, FC = Rs. ' + fmt(Math.round(FC)) + ' (capital / ' + years + ' years), VC1 = Rs. ' + fmt(Math.round(VC1 * 100) / 100) + ' per lamb, VC2 = 0.0002.');
  ctx.para('Lower break-even Q1 = ' + beQ1 + ' lambs (Rs. ' + fmt(be.sales1 == null ? 0 : be.sales1) + '). Upper break-even Q2 = ' + fmt(Math.round(beQ2)) + ' lambs. Farm capacity is ' + saleQty + ' lambs per year.');
...
export function sampleSheepInput(): SheepReportInput {
  return {
    language: 'en',
    breedName: 'Chokla',
...
export async function buildSampleSheepPdf(outPath: string): Promise<{ pages: number; bytes: number }> {
  const bytes = await buildSheepReport(sampleSheepInput());
